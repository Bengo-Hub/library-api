...
func (BibRecord) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("publication_place").Optional().Comment("Place of publication, e.g. 'Nairobi, Kenya'"),
		...
		field.String("cover_image_url").Optional().Comment("Front cover — relative media path or URL, resolved at read"),
		field.String("cover_back_image_url").Optional().Comment("Back cover — relative media path or URL"),
		field.JSON("authors", []string{}).Optional().Comment("Author display names (ordered)"),
		field.JSON("subjects", []string{}).Optional().Comment("Subject headings (display, denormalized)"),
		field.JSON("other_isbns", []string{}).Optional().Comment("Alternative/secondary ISBNs (other editions)"),
